...
class Model {
public:
    // vector = dynamic list container
    // <Type> = what kind of data the vector stores
    // Example: vector<Vertex> stores Vertex objects
    // model data
...
private:
    ...
    vector<Texture> loadMaterialTextures(aiMaterial *mat, aiTextureType type, string typeName) {
        ...
        for (unsigned int i = 0; i < mat->GetTextureCount(type); i++) {
            // Assume texture paths in model files are relative to the model’s directory; concatenate the model directory with the texture path to form the full path (absolute texture paths from some online models may need manual conversion to local paths).
            aiString str;
            mat->GetTexture(type, i, &str); //retrueves texture location
            ...
            if (!skip) {
                ...
                Texture texture;
                texture.id = TextureFromFile(str.C_Str(), this->directory); //loads with stb
